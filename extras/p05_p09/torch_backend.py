...
import torchvision
from .core import build_graph, cat, to_numpy, rel_path
...
def cifar10(root, valid_size=0.1):
    ...
    train_set, val_set = torch.utils.data.random_split(train_set,[num_train - split,split])

    train_set.data, train_set.targets = [np.array(train_set.dataset[i][0]) for i in train_set.indices], [train_set.dataset[i][1] for i in train_set.indices]

    val_set.data, val_set.targets = [np.array(val_set.dataset[i][0]) for i in val_set.indices], [val_set.dataset[i][1] for i in val_set.indices]

    return {
        'train': {'data': train_set.data, 'labels': train_set.targets},
        'val': {'data': val_set.data, 'labels': val_set.targets},
...
def Adam(weights, lr=0, weight_decay=0):
    return TorchOptimiser(weights, torch.optim.Adam, lr=lr, weight_decay=weight_decay)
